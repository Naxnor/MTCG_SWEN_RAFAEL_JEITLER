...
public class BattleService
{
    ...
    public int EnterLobby(int userId)
    {
        ...
        lock (lobbyLock)
        {
            ...
            if (!waitingEvents.ContainsKey(userId))
            {
        ...
        if (waitingEvents[userId].WaitOne(timeoutMilliseconds))
        {
            lock (lobbyLock)
            {
                if (lobbyQueue.Count > 0 && lobbyQueue.Peek() != userId)
        ...
        // No opponent was found or timeout occurred
        waitingEvents.Remove(userId);
        return 0;
    }

    
        public string StartBattle(int userId, string userName, int opponentId, string opponentName,
            IEnumerable<Card> userDeck, IEnumerable<Card> opponentDeck)
        {
            var userDeckList = userDeck.ToList(); 
        ...
        public string ExecuteBattle(int userId, string userName, int opponentId, string opponentName,
            IEnumerable<Card> userDeck, IEnumerable<Card> opponentDeck)
        {
            ...
            int opponentWins = 0;
            int round = 1;
            for (round = 1; round <= 100 && userCards.Count > 0 && opponentCards.Count > 0; round++)
            {
                ...
                battleLog.AppendLine(
                    $"Round {round}: {userCard.Name} (Damage: {outcome.UserEffectiveDamage}) vs {opponentCard.Name} (Damage: {outcome.OpponentEffectiveDamage})");
                ...
                battleLog.AppendLine(
                    $"Cards remaining - {userName}: {userCards.Count}, {opponentName}: {opponentCards.Count}");
            ...
            /*   if (round >=100) // uncomment for always draw in curl
           {
            ...
            {
                finalResult = "Battle ended in a draw " +
                              $"User {opponentName} won {opponentWins} rounds & User {userName} won {userWins} rounds";
            }
        ...
        SELECT elo, games FROM users WHERE id = @playerId;";
            ...
            using (var conn = new NpgsqlConnection(DBManager.ConnectionString))
            {
                ...
                UpdatePlayerStats(conn, player2Id, newElo2, games2 + 1,
        ...
        public float CalculateEffectiveDamage(Card attackingCard, Card defendingCard)
        {
            ...
            switch (attackingCard.Element)
            {
                ...
                case "Water":
                    ...
                    //if (defendingCard.Class == "Knight" && attackingCard.Element == "Spell") damage = 999999 ; // more balanced but more draws
                    break;
                case "Air":
            ...
            switch (attackingCard.Class)
            {
                ...
                case "Wizard":
                    ...
                    if (defendingCard.Class == "Goblin")
                        damage = 0; // Wizards mind-control Goblins, making them miss attacks
                    break;
                case "Dwarf":
                ...
                case "Kraken":
                    ...
                    break;
                case "Vampire":
                    if (defendingCard.Class == "Elf" || defendingCard.Class == "Dwarf" ||
                        defendingCard.Class == "Orc" ||
                        defendingCard.Class == "Goblin")
                    ...
                    break;
                case "Trap":
                    if (defendingCard.Class == "Troll") damage *= 2; // Trolls are dumb so they run into traps 
                    break;
